...
def display_webpage(buf, company_info, stock_data, company_news_df, company_sentiment_df, glassdoor_df, esg_stats_df):
    ...
    with tab2:
        ...
        # st.markdown(f"<b>Environmental Score: </b>{e_score}" , unsafe_allow_html=True)
        # st.markdown(f"<b>Social Score: </b>{s_score}", unsafe_allow_html=True)
        # st.markdown(f"<b>Governmental Score: </b>{g_score}", unsafe_allow_html=True)
        # st.markdown(f"<b>Backtested Weights Score (Rank 1 - Best Performance):</b> {backtested_weights}", unsafe_allow_html=True)
        # st.markdown(f"<b>Equal Weight Score (Rank 2 - Second Best Performance):</b> {equal_weights}", unsafe_allow_html=True)
        # st.markdown(f"<b>Industry-Specific Weights Score (Rank 3 - Third Best Performance):</b> {industry_specific_weights}", unsafe_allow_html=True)
        ...
        # "Likelihood of Recommendation of Company", "Employees' Approval of CEO")

        # Add traces to each subplot
        print(company_rating, job_recommendation, ceo_approval)
        ...
        if question:
            ...
            for entry in st.session_state.chat_history:
                with st.chat_message("human"):
                    ...
                    # print(entry["question"])
                with st.chat_message("ai"):
                    # st.write(f"{question}_Answer")
                    st.write(entry["answer"])

        #     # We need this because of feedback. That question above is a stopper. If user hits the feedback button, streamlit
        #     # reruns the code from top and we cannot enter back because of that chat_input.
